...
export default function Waitlist() {
  ...
  const emailConfig: EmailJSConfig = {
    serviceID: "service_u51rx5p"!,
    templateID: "template_h10huy8",
    publicKey: "uz1isZ-58c1jef7Ga",
  };
  ...
  const storeInWaitlist = (email: string) => {
    try {
      ...
      const emailExists = waitlistEmails.some(
  ...
  const handleWaitlistSubmission = async () => {
    ...
    try {
      ...
      const templateParams = {
        ...
        name: email.split("@")[0],
        message:
          "Your submission has been successfully recorded. We're excited to have you join our community as we prepare to launch MentalVerse, where mental wellness meets innovative technology.",
        reply_to: "mentalverseinc@gmail.com",
        from_name: "MentalVerse Team",
        title: "Welcome to MentalVerse Waitlist",
      };
      ...
      // Store email in localStorage
      const stored = storeInWaitlist(email);

      if (stored) {
        toast.success(
          "🎉 Welcome to MentalVerse! You're now on our waitlist. Check your email for confirmation."
        );
        setEmail("");
      } else {
        toast.error("This email is already on our waitlist!");
            ...
            <div className="relative bg-[#03050b51] backdrop-filter backdrop-blur-[14px] hover:bg-[#03050] rounded-[32px] p-4 sm:p-8 py-6 sm:py-10 overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8 w-full">
                ...
                <motion.button
                  ...
                  className={`px-8 py-3 bg-white text-black font-semibold rounded-full transition-all flex items-center gap-2 justify-center min-w-[140px] ${
                    isLoading || !email
