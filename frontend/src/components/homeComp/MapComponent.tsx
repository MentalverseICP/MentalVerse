...
const MapComponent: React.FC<prop> = ({className}) => {
  ...
  const filteredData = selectedCountry === "All" 
    ? data
    : data.filter(point => point.country === selectedCountry || point.country === "All");
    ...
    <div className={`h-full flex flex-row max-sm:flex-col justify-between bg-transparent rounded-3xl shadow-md border overflow-hidden ${className}`}>
          ...
          {filteredData.map((point, idx) => (
            <Marker key={idx} coordinates={point.coordinates}>
              <circle
                ...
                style={{
                  filter: selectedCountry === point.country ? 'drop-shadow(0px 0px 4px rgba(0,0,0,0.6))' : 'none'
                }}
              />
            </Marker>
          ))}
      ...
      <ul className="w-fit p-4 max-sm:p-3 max-[400px]:p-2 space-y-2 max-sm:space-y-1 text-sm grid items-center max-lg:pr-16 max-md:pr-10 max-sm:pr-0 max-sm:flex max-sm:flex-wrap max-sm:justify-between max-sm:gap-3 max-[400px]:gap-2">
        <li className="flex flex-col justify-between max-sm:min-w-[120px] max-[400px]:min-w-[100px]">
          <span className="font-bold uppercase text-sm max-sm:text-xs max-[400px]:text-[10px]">Recovery Zones</span>
          <span className="text-green-600 font-semibold max-sm:text-sm max-[400px]:text-xs">{stats.recovery}</span>
          <span className="rounded-md h-1 w-20 max-sm:w-16 max-[400px]:w-12 max-sm:h-0.5 bg-green-600" />
        </li>
        <li className="flex flex-col justify-between max-sm:min-w-[120px] max-[400px]:min-w-[100px]">
          <span className="font-bold uppercase text-sm max-sm:text-xs max-[400px]:text-[10px]">Neutral Zones</span>
          <span className="text-yellow-600 font-semibold max-sm:text-sm max-[400px]:text-xs">{stats.neutral}</span>
          <span className="rounded-md h-1 w-20 max-sm:w-16 max-[400px]:w-12 max-sm:h-0.5 bg-yellow-600" />
        </li>
        <li className="flex flex-col justify-between max-sm:min-w-[120px] max-[400px]:min-w-[100px]">
          <span className="font-bold uppercase text-sm max-sm:text-xs max-[400px]:text-[10px]">Affected Zones</span>
          <span className="text-[#ff0000] font-semibold max-sm:text-sm max-[400px]:text-xs">{stats.affected}</span>
          <span className="rounded-md h-1 w-20 max-sm:w-16 max-[400px]:w-12 max-sm:h-0.5 bg-[#ff0000]" />
        </li>
        <li className="flex flex-col justify-between max-sm:min-w-[120px] max-[400px]:min-w-[100px]">
          <span className="font-bold uppercase text-sm max-sm:text-xs max-[400px]:text-[10px]">Safe Zones</span>
          <span className="text-blue-600 font-semibold max-sm:text-sm max-[400px]:text-xs">{stats.safe}</span>
